...
/**
 * Refactor and add timer. You can play with the thread pool sizes (client &
 * server), the server arguments and delays.
 *
 * @author fguerry
 */
public class V07OperationTest {

    public static void main(String... args) {
        new V07OperationTest().run();
    }
    ...
    private ListenableFuture<List<Item>> getItems(Collection<Integer> ids) {
        Logger.getLogger(V07OperationTest.class.getName()).log(Level.INFO, "item ids : " + ids);
    ...
    private void processItems(List<Item> result) {
        for (Item item : result) {
            Logger.getLogger(V07OperationTest.class.getName()).log(Level.INFO, "Item : " + item);
        }
        end = System.currentTimeMillis();
        Logger.getLogger(V07OperationTest.class.getName()).log(Level.INFO, "duration : " + (end - begin));
    ...
    private JsonClientResponse<Item> getItem(Integer id) throws MalformedURLException, IOException {
        Logger.getLogger(V07OperationTest.class.getName()).log(Level.INFO, "call getItem " + id);
    ...
    private JsonClientResponse<ItemList> getItems() throws MalformedURLException, IOException {
        Logger.getLogger(V07OperationTest.class.getName()).log(Level.INFO, "call getItems");
